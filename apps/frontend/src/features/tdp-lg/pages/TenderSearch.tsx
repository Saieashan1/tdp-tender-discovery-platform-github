  ...
  return (
    <PageLayout>
      ...
      <div className="flex flex-col gap-6 mb-6">
        ...
        {showFilters && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="w-full mb-4">
              <button onClick={fetchTenders} className="px-4 py-2 rounded transition" style={{ backgroundColor: "#373292", color: "#ffffff" }}>
                Apply Filters
              </button>
            </div>
          </div>
        ...
        {/* Search input */}
        <input type="text" placeholder="Search for tenders..." className="border p-2 rounded w-full focus:outline-none focus:ring-2 focus:ring-blue-500" value={query} onChange={(e) => setQuery(e.target.value)} />
        ...
        <div className="mt-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {tenders.slice(0, visibleCount).map((tender) => (
              <div key={tender.referenceNumber} className="border p-4 rounded shadow-sm hover:shadow-md transition-shadow w-full h-[300px] overflow-hidden cursor-pointer relative" onClick={() => openTenderModal(tender)}>
                ...
                <div className="absolute bottom-0 left-0 w-full bg-gradient-to-t from-white to-transparent p-2 text-center font-bold" style={{ color: "#373292" }}>
                  Click to see more...
                </div>
              </div>
            ))}
          ...
          {visibleCount < tenders.length && (
            <div className="mt-4 text-center">
              <button onClick={() => setVisibleCount((prev) => prev + 6)} className="bg-blue-600 text-white px-4 py-2 rounded">
                View More
              </button>
            </div>
          )}
          ...
          <div className="flex items-center justify-center gap-4 mt-6">
            <button onClick={() => handlePageChange(pagination.page - 1)} disabled={pagination.page === 1} className="px-4 py-2 bg-blue-500 text-white rounded disabled:bg-gray-300">
              Previous
            </button>
            <span className="text-gray-600">
              Page {pagination.page} of {pagination.totalPages || 1}
            </span>
            <button onClick={() => handlePageChange(pagination.page + 1)} disabled={pagination.page === pagination.totalPages} className="px-4 py-2 bg-black text-white rounded disabled:bg-gray-300">
              Next
            </button>
          </div>
        </div>
